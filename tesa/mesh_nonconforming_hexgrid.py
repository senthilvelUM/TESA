"""
Type 2: Non-conforming hexagonal grid.

Generates a structured hexagonal grid mesh using meshgrid with offset rows,
...
def mesh_nonconforming_hexgrid(ms, job, run_dir=None, log_path=None, settings=None):
    """
    Generate a Type 2 non-conforming hexagonal grid mesh.

    Creates a structured hexagonal grid (meshgrid with offset rows),
    ...
    -------
    ms : Microstructure
        Updated with hexagonal grid mesh fields.
    """
    # ── Unpack settings ──────────────────────────────────────────────────
    if settings is None:
        settings = {}
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Step 1: Create structured hexagonal grid
    # ══════════════════════════════════════════════════════════════════════

    # Compute grid dimensions (ensure odd number of rows and columns)
    xs = round((maxx - minx) / h0)
    ys = round((maxy - miny) / (h0 * np.sqrt(3) / 2))
    if xs % 2 == 0:
        xs += 1
    if ys % 2 == 0:
        ys += 1
    ...
    # Offset even rows by h0/2 for hexagonal packing
    xx[1::2, :] = xx[1::2, :] + h0 / 2

    # Pin last column of offset rows back to maxx (right edge)
    xx[1::2, -1] = maxx

    # Flatten to point list
    p_hex = np.column_stack([xx.ravel(), yy.ravel()])

    # Remove points beyond the domain right boundary (shifted points may exceed maxx)
    p_hex = p_hex[p_hex[:, 0] <= maxx + 1e-10, :]

    # Add extra boundary nodes at minx for each offset row
    # (offset rows start at minx + h0/2, so we add a node at minx to match
    # the right edge pattern where Delaunay creates small fill triangles)
    offset_row_y = y_vals[1::2]
    extra_left = np.column_stack([np.full(len(offset_row_y), minx), offset_row_y])
    p_hex = np.vstack([p_hex, extra_left])

    if console_on:
        print(f"  Hex grid: {xs} cols x {ys} rows = {p_hex.shape[0]} points")
    ...
    print()
    print("=" * 62)
    print("  FINAL MESH SUMMARY (Type 2 — Non-conforming Uniform)")
    print("=" * 62)
    print(f"  Domain          : [{minx:.2f}, {maxx:.2f}] x [{miny:.2f}, {maxy:.2f}]")
    ...
    if log_path and vl in ("medium", "high"):
        with open(log_path, "a") as lf:
            lf.write("## Stage 2 — Mesh Generation (Type 2: Non-conforming Uniform)\n\n")
